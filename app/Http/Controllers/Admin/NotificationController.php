...
class NotificationController extends Controller
{
    ...
    public function store(Request $request)
    {
        try {
            ...
            $requestData['auther_id'] = 0;
            $requestData['content_type'] = 0;
            ...
            if (isset($requestData['image']) && $requestData['image'] != null) {
                ...
                $requestData['image'] = $this->common->saveImage($files, $this->folder, 'noti_');
    ...
    public function setting()
    {
        try {
            ...
            if ($params['data']) {
                return view('admin.notification.setting', $params);
            }
        } catch (Exception $e) {
            return response()->json(['status' => 400, 'errors' => $e->getMessage()]);
        }
    ...
    public function settingsave(Request $request)
    {
        try {
            $data = $request->all();
            $data['onesignal_apid'] = $data['onesignal_apid'] ?? '';
            ...
            return response()->json(['status' => 200, 'success' => __('label.success_add_notification_setting')]);
        } catch (Exception $e) {
            return response()->json(['status' => 400, 'errors' => $e->getMessage()]);
        }
    }
