...
class InstallController extends Controller
{
                ...
                if ($http_status == 200) {
                    ...
                    if (!empty($result) && $result->status == 200) {
                        ...
                        if ($result->result[0]->item->id == Item_Code()) {
                            ...
                            Set_Environment_Value('BUYER_USERNAME', $result->result[0]->buyer);
                            Set_Environment_Value('PURCHASE_STATUS', 0);

                            return redirect()->route('step3', ['token' => bcrypt('step_3')]);
                        } else {

                            session()->flash('error', __('label.whoops_the_code_you_entered_seems_to_be_for_a_different_product'));
                            return redirect()->route('step2', ['token' => bcrypt('step_2')]);
                        }
                    } else {

                        Set_Environment_Value('PURCHASE_CODE', $request[base64_decode('cHVyY2hhc2VfY29kZQ==')]);
                        Set_Environment_Value('BUYER_USERNAME', $request[base64_decode('dXNlcl9uYW1l')]);
                        Set_Environment_Value('PURCHASE_STATUS', 0);
                    ...
                    session()->flash('error', __('label.api_not_working_please_content_on_support_team'));
                    return redirect()->route('step2', ['token' => bcrypt('step_2')]);
                }
            } catch (Exception $e) {
    ...
    public function import_sql()
    {
        try {
            ...
            $sql_path = base_path('db/dt_ebook.sql');
